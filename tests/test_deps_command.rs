...
fn test_deps_task_with_no_dependencies() {
    ...
    let result = deps::execute(env.storage(), 1);
    assert!(result.is_ok());
}
...
fn test_deps_task_with_pending_dependency() {
    ...
    let result = deps::execute(env.storage(), 2);
    assert!(result.is_ok());
...
fn test_deps_all_dependencies_satisfied() {
    ...
    let result = deps::execute(env.storage(), 3);
    assert!(result.is_ok());
...
fn test_deps_invalid_id_zero() {
    ...
    let result = deps::execute(env.storage(), 0);
...
fn test_deps_invalid_id_out_of_range() {
    ...
    let result = deps::execute(env.storage(), 99);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("invalid"));
}

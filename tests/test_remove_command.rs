...
use rustodo::commands::{add, remove};
...
fn test_remove_single_task() {
    ...
    let result = remove::execute(env.storage(), 1, true);
    assert!(result.is_ok());
...
fn test_remove_id_zero_fails() {
    ...
    let result = remove::execute(env.storage(), 0, true);
...
fn test_remove_id_out_of_range_fails() {
    ...
    let result = remove::execute(env.storage(), 99, true);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("invalid"));
}

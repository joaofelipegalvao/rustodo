...
**🎯 Goal:** Move task data from current working directory to OS-appropriate global location
...
❌ **Fragmented task lists** - Tasks scattered across directories  
❌ **Lost tasks** - Easy to forget which directory has which tasks  
❌ **Inconsistent state** - Running from different locations shows different data  
❌ **Non-standard behavior** - Real CLIs don't work this way  
❌ **Backup complexity** - Must back up multiple `todos.json` files  
...
✅ **Single source of truth** - One task list across all directories  
✅ **Standard behavior** - Matches professional CLI applications  
✅ **Easy backups** - One known location to back up  
✅ **XDG compliance** - Follows platform conventions  
✅ **Portable** - Works correctly on Linux, macOS, and Windows  
...
1. **Get project directories:**
   ...
   ```rust
   let project_dirs = ProjectDirs::from("", "", "todo-cli")
   ...
   - `.context()` adds error context if it fails
...
3. **Create directory unconditionally:**
   ...
   - Error context includes the actual directory path

4. **Build path to todos.json:**
   ...
   - Convert `&Path` to owned `PathBuf`
   - Push filename onto path
...
fn load_tasks() -> Result<Vec<Task>> {
    match fs::read_to_string("todos.json") {  // ← Hardcoded filename
...
2. **Simplified error messages:**
   ...
   - Error context doesn't repeat the path (it's in the chain)
   - Clearer, more concise messages

3. **Read error with path context:**
   ...
   - Shows where the file was attempted to be read
   - User knows the exact location that failed
...
fn save_tasks(tasks: &[Task]) -> Result<()> {
    ...
    fs::write("todos.json", json)  // ← Hardcoded filename
...
- Error message includes actual file path
- Error message mentions permission check
...
```rust
Commands::Clear => {
    if fs::metadata("todos.json").is_ok() {  // ← Hardcoded filename
...
```rust
Commands::Clear => {
    let path = get_date_file_path()?;  // ← Dynamic path
...
**🎉 Your tasks are now properly managed like a professional CLI application!**
